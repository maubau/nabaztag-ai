...
> **How to read this document.** v2.2 and earlier were a *plan*: they described what was going to be built. From v2.3 on, most of this is **built and hardware-verified**, and the plan was wrong in places — usefully wrong. Where reality overruled the design, the old text is not quietly deleted: it is marked and the evidence that overruled it is cited. `docs/OJN_API_NOTES.md` is the running lab notebook (24 numbered findings, each with the hardware round that produced it); this document is the shape of the system. When the two disagree, the notebook is newer.

**Changelog**

- **v2.2 → v2.3:** reconciled with ~six weeks of hardware. **Two non-goals were overturned by evidence and one was confirmed the hard way.** Barge-in (§3) was a non-goal justified by the missing AEC reference; the reSpeaker XVF3800's on-chip AEC was then *measured* and a full-duplex **realtime mode** shipped (§6.2.10), so the rabbit can now be interrupted mid-sentence — half-duplex remains the default and the fallback. Audio output became **switchable** between the rabbit's own speaker and a Bolt device (§6.2.6), because barge-in needs playback that can be cancelled and the 2006 decoder cannot be. Conversely "no streaming audio through the rabbit" (§3) is now a *proven* limit, not a caution: the MTL decoder buffers each file to EOF (Gate L3, hardware-rejected). Also: the LLM is **OpenAI**, not Claude (§6.2.5 — the interface stayed provider-neutral, the earlier text simply had not caught up); STT is **Deepgram Flux** with provider-side end-of-turn (§6.2.4); TTS is **self-hosted Piper** after an on-hardware voice A/B (§6.2.6); the choreography plugin foreseen in §6.1 was **not needed** (Gate G0); recovery, systemd units and the runtime's single-owner rules are documented (§6.7); phase statuses updated; §10 marked historical.
...
- **No true streaming audio through the rabbit speaker — now a PROVEN limit, not a precaution.** Gate L3 dribbled a single MP3 to the rabbit over 7.8 s: playback began only *after* the transfer completed, while `tcpdump` confirmed Apache had delivered it progressively. The MTL decoder buffers to EOF; the fault is in the 2006 firmware, not in our delivery path (`OJN_API_NOTES` #21). Replies are therefore complete MP3s, optionally sentence-level and queued (§6.2.6). **Do not build a progressive/streaming path for the rabbit speaker.**  
- No local LLM (Bolt runs local STT; the LLM is a cloud API — the local-LLM chapter belongs to the VENTUNO Q profile later).  
- ~~No barge-in: half-duplex only.~~ **OVERTURNED by measurement (v2.3).** The non-goal rested on "there is no AEC reference for the rabbit's speaker" — true, and still true *for that speaker*. Routing output through the reSpeaker instead gives the XVF3800 the far-end reference over USB, and its on-chip AEC was then verified rather than assumed: on ch0 the echo residual sits **6.7 dB below the room noise floor** while near-end speech is **+58.6 dB over that residual** (`brain/scripts/aec-probe.py`, July 2026). Full-duplex realtime mode followed (§6.2.10). **Half-duplex remains the default** and the automatic fallback: barge-in is available only with `audio_out.backend: local`.  
...
4. **STT — three profiles behind the `STTProvider` interface, selected in `config.yaml`:**  
   - `flux` (**production**): **Deepgram Flux**, which does recognition *and* end-of-turn detection in one pass. This replaced ~1875 ms of fixed client-side cost (1600 ms Silero window \+ ~275 ms nova-3 finalisation) with the provider's own EndOfTurn, measured on hardware at **~214–226 ms** from the moment the user stops speaking. Providers advertise `detects_end_of_turn` and the pipeline branches on it, so the two endpointing styles are not tangled together. `EagerEndOfTurn` is recorded for diagnostics but **never acted on** — no speculative dispatch.  
...
5. **Agent loop:** LLM behind a provider-neutral `LLMProvider` interface (`llm/base.py`). **OpenAI is the active provider** (Responses API, streaming \+ function calling; model configurable via `llm.model`, never hardcoded) — the earlier "Claude API" wording in v2.2 was a leftover from v1.0, and the interface is what actually keeps the choice swappable. Production is **`gpt-5.4-mini` \+ `reasoning_effort: low`**, from a real A/B (`brain/scripts/llm-bench.py`, 3 runs/scenario): median final-text 1615 ms vs 2162 ms at effort `none`, equal quality. `gpt-5.4-nano` was ruled out — slower *and* more tool rounds, despite being marketed as the fast option. **Decide on `final_text`, not on first token:** TTS cannot start until the text is complete, so a faster first token buys nothing here. Rolling history (`llm.max_history_turns`), personality prompt (`prompts/system.md`), tool rounds capped (`llm.max_tool_rounds`). `OPENAI_API_KEY` comes only from the environment and is never logged.  
6. **TTS: self-hosted Piper (production), Deepgram Aura as automatic fallback.** Chosen by listening *through the rabbit*, which is the only test that counts — its speaker is small and its decoder ancient. Italian `it_IT-paola-medium` at `length_scale` **1.25** (the default pace is too fast) and English `en_GB-alba-medium` at **1.0**; `en_US-sam-medium` was rejected as flat. Piper also fixed the persistently low volume that dogged Aura. Voice is routed by **the STT's own detected language**, never by text heuristics. Piper runs as a persistent HTTP server per language — the engine is GPL-3.0 and stays an external localhost process, with no code vendored. Alba is CC BY 4.0: **attribution is required** (Alba dataset, University of Edinburgh). ElevenLabs was tried and not promoted (poor Italian, very low volume). Output is a complete MP3 in `www/audio/`, served by Apache (§5); long replies may be split into sentence-level files queued sequentially.  
7. **Half-duplex gate:** mic pipeline pauses from the OJN play command until estimated playback end (+300 ms guard), keyed on `BodyController.audio_busy` so queued-but-not-yet-started audio counts too. Capture is **drained, not paused**, during processing — an unread queue starved the pipeline into "capture queue full".  
8. **DoA behavior:** on wake word, read DoA angle → map to an ear gesture "turning toward the speaker" before the listening pose. **Choreography-only, never `posleft`/`posright`:** that path makes the firmware play a long carillon (probe #7, source-confirmed — any ear-position change in an AmbientPacket triggers `controlsound midi_communion`). DoA reads are time-bounded so a USB stall cannot freeze the feedback, and fail open.  
...
10. **Realtime mode (`conversation.mode: realtime`) — full-duplex, the v2.3 addition.** One WebSocket (OpenAI Realtime) carries audio both ways: the wake word opens a **continuous conversation** that is not re-armed per turn, and the user can talk over the rabbit. **Requires `audio_out.backend: local`** and the runtime refuses the combination on the rabbit backend rather than half-supporting it. On `speech_started` the local stream is cut and `conversation.item.truncate` tells the model how much was *actually heard* — get that wrong and its transcript keeps words the user never heard, and every later turn reasons from fiction. **What it costs:** the model generates its own voice, so Paola/Alba are not used and the Italian is unverified — which is exactly why this is a mode, not a replacement. Any WebSocket/API failure re-arms `turn_based` in place, without restarting the service.
11. **Latency, measured and then closed.** Second-turn hardware timings: first token **824 ms**, audio queued **2.4 s** (first turn ~4.4 s cold). Residual variability is LLM-dominated. **Decision (July 2026): performance work is CLOSED for v1.** Explicitly not to be built — speculative/anticipatory TTS, progressive MP3 (already hardware-rejected, §3), further LLM benchmarking. Re-open only if a hardware or firmware assumption changes.
...
> **Hardware finding that constrains every indicator built on top of this (`OJN_API_NOTES` #8).** `submit()` confirms **local enqueue only.** Nothing reports execution on the rabbit, and a new `chor=` does **not** replace the running one — it **queues behind it**, with no cancel or replace anywhere in the API. This was found the expensive way: a UX loop resubmitting a ~1.9 s animation every cycle handed a 30 s conversation about fifteen queued animations, so after the conversation had provably ended — logs at `REARMED`, session closed — the rabbit kept animating for another 20–30 s, working through a backlog we could not recall.
>
...
**Exactly one consumer of the microphone**, always. Every failure path in the realtime branch is bounded for this reason and no other: a teardown step that hangs stops the capture being drained, and the run is over ("capture queue full" for the rest of the session). Where a bounded wait is not enough — a PortAudio stream that will not release the device — the runtime **terminates** and lets systemd's `Restart=always` rebuild the process, because the alternative is worse than a restart: the wake word fires, the LEDs light, the reply is silent, and every log line says the system is healthy.

**systemd:** `deploy/nabaztag-runtime.service` and `deploy/nabaztag-recovery.service`, installed and enabled at boot on the Bolt. (`StartLimitIntervalSec`/`StartLimitBurst` belong to `[Unit]`, not `[Service]` — systemd ignores them silently in the wrong section.)

**Recovery — deliberately narrow.** What looked like an unrecoverable XMPP wedge turned out to be a **Wi-Fi disassociation for inactivity that the Bolt self-heals**: after a week the SSID was gone and the interface counters frozen while `hostapd` still reported active; restarting *only* `hostapd` brought the rabbit back in ~2 s. The ghost XMPP sockets and the stale neighbour entry were symptoms, not causes. So `deploy/rabbit_recovery.py` restarts **hostapd and nothing else**, only when three signals coincide (rabbit not associated \+ last HTTP old \+ stale `:5222` socket), with a cooldown, a per-outage cap and an hourly hold so it cannot loop when the rabbit is simply switched off. **It never restarts OJN and never reboots the Bolt.** The decision logic is a pure function with no I/O, so it is fully testable; "last HTTP" is parsed from the newest Apache log line whose client is exactly the rabbit's IP — file mtime is *not* a valid proxy, as our own `curl` to localhost demonstrated by refreshing it hours after the rabbit's real last request.
